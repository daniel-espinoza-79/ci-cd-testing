...
export async function middleware(request: NextRequest) {
  ...
  if (!token) {
    return NextResponse.redirect(
      new URL(DEFAULT_UNAUTHORIZED_REDIRECT, request.url)
    );
  }
  ...
  try {
    ...
    const hasAccess = permissions.paths.some((path) =>
      matchPath(currentPath, path)
    );
...
export const config = {
  matcher: [
    "/((?!api|_next/static|_next/image|favicon.ico|manifest.json|icons-192.png|icons-256.png|icons-512.png|Isotipo-Colores.png).*)",
  ],
};
